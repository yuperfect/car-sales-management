<template>
  <div>
    <!-- ====== 未设置身份：引导提示 ====== -->
    <div v-if="!identified" class="card" style="max-width: 480px;">
...
const identified = ref(isIdentified())
const errMsg = ref('')
...
async function loadProfile() {
  ...
  try {
    ...
    // 同步 identified 状态
    identified.value = !!(data && data.username)
  } catch (e) {
...
async function handleSwitchUser() {
  ...
  try {
    ...
    showSwitchModal.value = false
    isEditing.value = false
    identified.value = true  // 立即切换视图，不等 loadProfile
    await loadProfile()
