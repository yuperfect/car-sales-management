<template>
  <div>
    <div class="card">
      <div class="card-header">试驾预约管理</div>
        ...
        <table>
          ...
          <tbody>
            ...
            <tr v-if="list.length === 0">
              <td :colspan="activeTab === 'pending' ? 8 : 7" style="text-align: center; color: #999;">
                暂无{{ activeTab === 'pending' ? '待确认' : '' }}试驾预约
              </td>
            </tr>
          </tbody>
        </table>
...
import { fetchTestDrives, confirmTestDrive } from '../api/index.js'
...
async function loadData() {
  ...
  try {
    ...
    list.value = await fetchTestDrives(params)
  } catch (e) {
    error.value = '获取试驾预约列表失败：' + (e.message || '网络错误')
  } finally {
    loading.value = false
  }
}
