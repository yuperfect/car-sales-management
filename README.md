...
项目根目录提供 **3 个独立 exe 工具**，双击即可运行，无需安装任何运行时环境（.NET 8 已编译为独立程序）。

### 首次运行必做：`配置检查.exe`

首次使用或更换电脑时，先运行环境检查工具：

1. **双击 `配置检查.exe`**
2. 工具自动检查以下 **11 项**环境（整个过程约 1 分钟）：

| # | 检查项 | 自动修复 |
|---|--------|---------|
| 1 | ✅ JDK 版本 | 仅检查，无修复 |
| 2 | ✅ Node.js + npm | 仅检查，无修复 |
| 3 | ✅ 后端 Jar 包 | 缺失时询问是否自动编译 |
| 4 | ✅ 前端 node_modules | 缺失时询问是否自动安装 |
| 5 | ✅ 端口占用 (8080/5173/3000) | 仅检查，无修复 |
| 6 | ✅ MySQL 数据库连接 | 提示输入账号密码测试 |
| 7 | ✅ 数据库 `car_sales_db` | 缺失时自动创建 |
| 8 | ✅ 数据表结构 (schema.sql) | 缺失时询问是否建表 |
| 9 | ✅ 种子数据 (data.sql) | 缺失时询问是否插入 |
| 10 | ✅ application.yml 配置 | 自动更新数据库连接信息 |
| 11 | ✅ 前端构建配置 | 仅检查，无修复 |

> 关键操作（编译 jar、安装依赖、建表、插入数据）均会询问 `(Y/n)`，安全可控。

### 启动系统

```powershell
# 方式一：双击 启动系统.exe
# 方式二：双击 启动系统.bat（控制台窗口，可观察启动日志）

# 方式三：命令行
.\启动系统.exe
```

自动执行：检查端口 → 编译后端 → 启动后端 → 启动客户端 → 启动管理端 → 验证 API 健康 → 打开浏览器
...
双击 **`关闭系统.exe`** 一键停止所有服务。

### 完整使用流程

```
第一步: 双击 配置检查.exe    → 确保环境就绪
第二步: 双击 启动系统.exe     → 一键启动所有服务
第三步: 使用完毕 → 双击 关闭系统.exe
```
...
├── plan/                         # 敏捷交付文档
├── quality/                      # 质量文档
├── 启动系统.exe                  # 一键启动所有服务（独立 exe）
├── 配置检查.exe                  # 环境配置检查工具（独立 exe）
├── 关闭系统.exe                  # 一键关闭所有服务（独立 exe）
├── start-all.ps1                 # 一键启动脚本（供 exe 内部调用）
├── stop-all.ps1                  # 一键停止脚本（供 exe 内部调用）
├── status.ps1                    # 状态查看脚本
└── README.md                     # 本文件
```
...
> **⚠️ 如果 MySQL 密码不同**，运行 `配置检查.exe` 时会提示你输入用户名和密码，并自动更新配置文件。无需手动修改。
...
自动测试所有 API 接口以及客户端/管理端的核心功能。

> 测试结果可导出为 HTML 报告，在浏览器中查看详细日志。
